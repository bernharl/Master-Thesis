...
\begin{figure}
    ...
    \caption{Best predictions by Mixed$_\text{lstm}$ on CAMELS-GB (top) \citep{CAMELS_GB} 
...
\begin{table}
    \centering
    \caption{Attribute importance ranked by median importance on CAMELS-GB \citep{CAMELS_GB} 
...
ranked by median importance as found using the permutation test.
...
on CAMELS around epoch ??. The epoch optimized for both datasets which is shown in 
Figure \ref{transfer CDF validation} is epoch ??. For Transfer$_\text{GB, none}$ 
...
epoch found for this model on both datasets is epoch ??.
\begin{figure}
\includegraphics{{results_section/transfer/predictions_best_train_us_no_static}.pdf}
\caption{Best predictions by Transfer$_\text{US, lstm, none}$ on CAMELS-GB (top) 
...
and US$_\text{Kratzert}$ shown in Figure \ref{best prediction kratzert}.
...
\begin{figure}
    ...
    \caption{From the top top: GB$_\text{lstm, all}$, US$_\text{Kratzert}$, Mixed$_\text{lstm}$ 
