...
\begin{table}
    \centering
    \caption{Attribute importance ranked by median as found using the permutation 
    test on Mixed$_\text{lstm}$.}
    \resizebox{\textwidth}{!}{%
...
As both cases show that the transfer learning works better without static attributes 
...
Second from the top in Figure \ref{test cdf} shows US$_\text{Kratzert}$ refit on the full training set 
...
Third from the top in Figure \ref{test cdf} we see 
