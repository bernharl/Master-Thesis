...
\begin{table}
    ...
    in this section according to the permutation feature importance algorithm. The columns are percentiles.}
...
permutation algorithm described in Chapter \ref{Feature selection}. We observe that the 
...
\begin{table}
    ...
    according to the permutation algorithm. The columns are percentiles.}
    \input{{tables/results_section/all_features_concat_seq_len_importance}.tex}
    \label{all concat table}
\end{table}

The top 20 importances found by US$_\text{lstm, all}$ (cross validated) using the permutation algorithm 
...
\begin{table}
    ...
    algorithm on Mixed$_\text{lstm}$. Each percentile column is split into two subcolumns, 
...
ranked by median importance as found using the permutation algorithm. Sorted by median 
...
\begin{figure}
    ...
    \caption[Rainfall-runoff ratio scatter plot]{Rainfall-runoff ratio scatter plotted against NSE values per basin. 
...
\includegraphics{{results_section/transfer/training_progress}.pdf}
    \caption[Training progress Transfer$_\text{US, lstm, none}$]{Cross validated performance per epoch of Transfer$_\text{US, none}$ and Transfer$_\text{GB, none}$ 
...
These models are denoted as Transfer$_\text{GB/US, model-type}$
...
\begin{table}
    \centering
    \caption[Summary NSE table of best models.]{Summary table showing median validation and test NSE values of the 
