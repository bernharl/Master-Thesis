...
\begin{figure}
    \includegraphics{{results_section/camels_gb/cheat/overfit_performance}.pdf}
\caption{Performance of model trained on CAMELS-GB with access to attributes 
    derived from the outcome.}
    \label{cheat model}
\end{figure}
We begin by presenting the performance of a model which has access to features 
derived from streamflow. The rationale for this is described in Chapter \ref{Feature selection}.
Figure \ref{cheat model} shows the performance of a model using all time series and 
all numerical static attributes of CAMELS-GB.
