...
First we split the data basin-wise into train (75\%) and test (25\%) sets. The test 
set is discarded and only used in the very end to show test performance of our best 
performing models for each scenario. We use cross validation on the training set 
to evaluate model performance. More details on this is shown in the following 
algorithm:
\begin{figure}
\centering
...
\begin{enumerate}
    ...
    \item Repeat the following five times, each time using a different 1/5 of the 
        ...
        \item Split each training time series into several parts, the length 
            of which is decided by the sequence length variable $s$. A minibatch 
            consists of a batch size $s$ amount of these $s$ long timeseries. This 
                structure is shown in Figure \ref{minibatch}.
