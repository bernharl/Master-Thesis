...
advances in time  \cite{bengio1994learning} \cite{graves2012long}. The LSTM model is designed to fix this, 
...
\end{tikzpicture}
\caption{A single basic LSTM cell as proposed in \cite{hochreiter1997long}. The 
Tikz code for creating this figure is released under the permissive Beerware license 
and is therefore taken with permission from \cite{tikzlstm}
Visually we see the point of the Cell State $C$ here is to not retain more information 
...
\label{LSTM figure}
\end{figure}
It is difficult to describe an LSTM purely through mathematics, therefore we 
include Figure \ref{LSTM figure}. In the figure lines meeting means concatenation, 
the boxes means a neural network layer. The boxed $\sigma$ for instance indicates 
a neural network with the sigmoid (\ref{sigmoid}) activation function.
...
model architecture itself is designed to keep track of these. This means that the 
LSTM has two clear advantages:
\begin{enumerate}
