...
A method to remove correlated static feature that is fairly simple to use is clustering on 
\begin{Huge}
    Spearman or Pearson, please decide!!
\end{Huge}    rank-order correlations \cite{permutation_clustering}. The most 
well known algorithm for finding correlation is called Pearson correlation. It has 
...
\begin{figure}
	...
	\caption{Example of a hierarchical plot created on some simple gaussian distributions. \large{REMEMBER TO DOUBLE CHECK WHETHER YOU WANT NP.ABS OF PEARSON CORR!!}}
    \label{wald example}
\end{figure}
