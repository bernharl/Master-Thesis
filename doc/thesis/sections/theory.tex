...
The name stems from the equation for infiltration capacity
\begin{equation}
    i = i_m \left[ 1 - (1-A)^{1/b_i}  \right]
\end{equation}
where $i$ is the infiltration capacity, $i_m$ is the maximum infiltration capacity
defined as 
\begin{equation}
    i_m = (1+b_i)\theta_s|z|. \cite{VIC2L}
\end{equation}
...
as 
{\Large FIND OUT WHY THESE ARE SO DIFFERENT}
\begin{equation}
Q_d\left[ N+1 \right] \cdot \Delta t = 
\begin{cases}
	P \cdot \Delta t - W_1^c + W_1^t \left[ N+1 \right], &  i_0 + P \Delta t \geq i_m\\
	P \cdot \Delta t - W_1^c + W_1^t \left[ N+1 \right] \\ + W_1^c \left[  1 - 
    \frac{i_0 + P \cdot \Delta t}{i_m} \right]^{l+b_i}, & i_0 + P \cdot \Delta t \leq i_m
\end{cases}
\end{equation}
{\Large $\theta$ is volumetric water content. $W$ is soil moisture content. Are these the same?? Seems like $W_c=\theta_s |z|$.}
\begin{equation}
Q_d\left[ N+1 \right] \cdot \Delta t = 
\begin{cases}
...
\end{cases}
\end{equation}
\footnote{To better fit with the notation of this thesis we have gone from the original 
notation from \cite{VIC} where time steps are noted with $W_i^+$ and $W_i^-$ to a more 
standard notation of $W_i^t$ and $W_i^{t-1}$.}

Information about the soil moisture of the previous time step is stored in the 
relationship between previous upper layer soil moisture $W_1^t$ and the new 
upper layer soil moisture 
\begin{equation}
    W_1^{t+1} \left[N+1 \right] = W_1^t \left[ N+1  \right] + \left( P - Q_d\left[ 
...
\begin{equation}
	Q_b'[N+1] = 
	\begin{cases}
\frac{D_s D_m}{W_s W_2^c}W_2^t[N+1], &  W_2^t\left[ N+1 \right] \in [0, W_s W_2^c] \\
\frac{D_sD_m}{W_sW_2^c}W_2^t\left[ N+1  \right]\\ + \left( D_M - \frac{D_sD_m}{W_s}  \right) 
\\ \cdot \left( \frac{W_2^t\left[ N+1  \right] - W_sW_2^c}{W_2^c - W_sW_2^c}  \right)^2, 
& W_2^t\left[ N+1  \right] \geq W_sW_2^c
	\end{cases}
\end{equation}

Updating the lower layer soil moisture is then described as 
\begin{align}
    W_2^+\left[ N + 1 \right] &= \text{min}\left( W_2^t \left[N+1\right] \right. \nonumber
...
\begin{equation}
    Q_b''\left[ N + 1 \right] =  W_2^t \left[N+1\right]+\left(Q_{12}
...
\begin{equation}
    ...
    \begin{cases}
        Q_b', & W_2^{t+1} < W_2^c \\
        Q_b' + Q_b'', & W_2^{t+1} = W_2^c
    \end{cases}
\end{equation}

For other ground covers than bare soil $Q\left[ 1...N \right]$ the model is slightly 
modified. We do not present the details of this in this thesis and instead shift 
...
have added to and improved these representations. The original VIC also did not account 
properly for frozen soil either, the effect of which is important in cold climates.
The newest version of VIC \cite{VIC5} uses a modification from \cite{VICFrozenSoil}
to model frozen soil. 

The model is based on the heat equation for frozen soil, which is defined as 
\begin{equation}
...
The water content of layer $i$ when ice is present is then described as 
\begin{equation}
    W_i = W_i^c \left[ \left( \frac{1}{g \psi_e} \right) \left( \frac{L_fT}{T+273.16
    \text{ K}}  \right) \right]^{-B_p}.
\end{equation}
...
parameters that need to be set. Many of these parameters are given a-priori 
...
motivated values \cite{VICbench}. 
...
\begin{figure}
	...
	function form is in part inspired by Figure 2.11 in \cite{elemstatlearn}. For 
