...
of the model is presented here for completeness. It works by splitting the catchment 
area into $N$ grid points and the ground (vertically) into two soil layers. 

The runoff modelling is split into three parts:
\begin{enumerate}
	\item Surface runoff from bare soil
	\item Subsurface runoff from bare soil
	\item Covered surfaces (Vegetation, snow, etc.)
\end{enumerate}
...
\begin{cases}
    ...
    \frac{1 + P \cdot \Delta t}{i_m} \right]^{l+b_i}, & i + P \cdot \Delta t \leq i_m
\end{cases}
\end{equation}
...
\left( \frac{W_2^-\left[ N+1  \right] - W_sW_2^c}{W_2^c - W_sW_2^c}  \right), 
