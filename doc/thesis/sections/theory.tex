...
\begin{enumerate}
    \item Conceptual models: These models can be referred to as so called 
		"grey box" models, as many important physical effects are replaced
		by parameters that have to be optimized. Classically this is viewed as 
		bad for generalization, as there is no inherent physical meaning behind an 
		optimized parameter.
	\item Process-based models: These models are in a larger degree based on physical 
		...
		they should ideally be given physical meaning. This in spite of indications 
		of increased performance from optimizing more parameters, forgoing their 
		original physical meaning \citep{VICbench,BiasVarianceVIC}.
\end{enumerate}
...
The model focuses on conceptually modelling streamflow, using either real or modelled 
precipitation and evapotranspiration data as input. It uses relatively simple differential equations to 
keep track of the water collection in different layers of soil. This is not derived 
from any physical equation, and is what is often known as a so called "bucket model", 
i.e. it keeps track of stored water in layers of soil and needs parameters such as 
drainage coefficients to be optimized to properly be able to model the streamflow 
in a basin. As well as needing evapotranspiration, the model also needs to be 
...
While this model's treatment of soil layers as "buckets" is not entirely inconsistent 
with real world physics, the bucket behaviour is not based on a physical equation. 
The models also needs to optimize many parameters, making said parameters harder 
to assign any physical meaning to. 
...
The runoff modelling is split into several parts. Some examples are:
\begin{enumerate}
...
The baseflow $Q_b$ is defined as \citep{VIC2L}
\begin{equation}
...
\begin{figure}
    ...
    \caption[The bias-variance tradeoff.]{Illustration of the bias-variance tradeoff. The analogy to physics-
	...
	function form is in part inspired by Figure 2.11 in \citet{elemstatlearn}. For 
    real world data the effect is not guaranteed to be this drastic.}
	\label{Bias Variance rainfall}
\end{figure}
In \citet{VICbench} the concept of model flexibility is mentioned. This is 
...
a more data driven conceptual model would have higher variance. 

The concept of 
the bias-variance tradeoff concept is illustrated  in Figure 
\ref{Bias Variance rainfall}. When a model is to simple it fails to properly capture 
the behavior of a given system and the error increases. When a model is too complex 
it starts fitting the train set perfectly and loses all generalizability, the test 
error therefore increases.

There are many ways to combat this issue. The most obvious solution is to increase the 
amount of data a model is trained on. This reduces the amount of model parameters 
compared to the model of data points and makes the model less likely to overfit. 
Another way is to forcefully increase the bias of a model with high complexity 
through a process known as regularization. Regularization comes in many forms, often 
specific to the models that are being regularized.
...
be beneficial to employ in such a case \citep{elemstatlearn} \citep{handson}. The 
...
absolute minimum of $F(\mathbf{x})$, the algorithm often converges at local minima \citep{elemstatlearn}.
...
\end{align}
as shown by \citet{fysstkweek40}.
...
In Figure \ref{RNN figure} we see that the output at time step $t$ is represented 
...
hand is more general and can find non-linear correlation \citep{spearman}. After 
finding the correlations of the features one then uses a clustering method to 
visualize these correlations. The source at \citet{permutation_clustering} suggests 
...
While \citet{permutation_clustering} suggests using the Spearman rank order correlation,
