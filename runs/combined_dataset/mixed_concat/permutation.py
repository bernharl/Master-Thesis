...
for i in range(0, 5):
    ...
    cv_dir = list((Path("/work") / "bernharl" / "mixed_concat" / f"{i}").glob("*"))
    if len(cv_dir) != 1:
    ...
    cfg["train_basin_file"] = Path(
        f"../mixed/cv/cross_validation_seed_19970204/{i}/basins_train.txt"
    )
    cfg["val_basin_file"] = Path(
        f"../mixed/cv/cross_validation_seed_19970204/{i}/basins_val.txt"
    )
    permutation_score = permutation_test(cfg, k=2, epoch=15)
